  ...
  while(i<4):
    ...
    if(edge[0]==31 and edge[1] == 44):
      if(test[edge[0]-1] == color2 and test[edge[1]-1] == color1):
        ...
        test = Top_counterclockwise(test)
        test = Right_clockwise(test)
        test = Top_clockwise(test)
...
def Top_Cross(test):
  ...
  if(all_yellow):
    ...
    if(test[46] == test[34] == 5):
      test = Move_of_Top_cross(test)
    elif(test[28] == test[40] == 5):
      test = Top_clockwise(test)
      test = Move_of_Top_cross(test)
    elif(test[46] == test[28] == 5):
      test = Move_of_Top_cross_L(test)
...
def build_top_edge(test):
  ...
  if(test[26] == color2):
    test = change_view(test)
    test = change_view(test)
    test = Move_of_Top_edge(test)
    test = change_view(change_view(change_view(test)))
    test = Move_of_Top_edge(test)
  elif(test[25] == color2):
    test = change_view(test)
    test = Move_of_Top_edge(test)
  ...
  if(test[24] == color2):
    test =test
    test = change_view(change_view(test))
  elif(test[25] == color2):
    test = change_view(test)
    test = Move_of_Top_edge(test)
    test = change_view(test)
...
def Build_top_cor_sep(test):
  j = 0
  
  for i in range(4):
    while(test[16]!=5):
      test = Move_top_corner_sepe(test)
      j+=1
      # f.write(str(test)+"\n")
...
def solver(test):
  test = buildcross(test)
  test = Build_top(test)
  test = Build_2nd_layer(test)
  test = Top_Cross(test) 
  test = build_top_edge(test)
  test = Build_top_corner(test)
  # print_all_faces(test)
  test = Build_top_cor_sep(test)
  return test
def checker(test,f):
  check_list = test[0:6]
  for i in range(1,8):
